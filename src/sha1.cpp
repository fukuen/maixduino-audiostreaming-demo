...
void SHA1::update(StringStream &is)
{
    String s = is.str();
    for (int i = 0; i < s.length(); i += BLOCK_BYTES)
    {
//        char sbuf[BLOCK_BYTES];
//        int bytes = is.readBytes(sbuf, (size_t)(BLOCK_BYTES - buffer.length()));
//        buffer.append(sbuf, (std::size_t)is.gcount());
        buffer = s.substring(i, i + BLOCK_BYTES);
        if (buffer.length() < BLOCK_BYTES)
        {
            return;
        }
        uint32_t block[BLOCK_INTS];
        buffer_to_block(buffer.c_str(), block);
...
String SHA1::final()
{
    ...
    uint64_t total_bits = (transforms*BLOCK_BYTES + buffer.length()) * 8;

    /* Padding */
    buffer += (char)0x80;
    size_t orig_size = buffer.length();
    while (buffer.length() < BLOCK_BYTES)
    {
        buffer += (char)0x00;
    }
...
//    String result;
    ...
    {
//        Serial.println(digest[i], HEX);
//        result.print(digest[i], HEX);
//        result.print((char *)digest[i]);
//        result += digest[i];
        unsigned char buf[4];
        buf[0] = digest[i] >> 24;
        buf[1] = digest[i] >> 16;
        buf[2] = digest[i] >> 8;
        buf[3] = digest[i];
        result.write((uint8_t)buf[0]);
        result.write((uint8_t)buf[1]);
        result.write((uint8_t)buf[2]);
        result.write((uint8_t)buf[3]);
